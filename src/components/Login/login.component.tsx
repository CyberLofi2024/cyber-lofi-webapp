...
function LoginComponent() {
  ...
  const onSubmit: SubmitHandler<IUserLogin> = (data) => {
    if (data.email === "admin@cyberlofi.com" && data.password === "admin123") {
      setLoginData(data);
      toast.success("Welcome to KUBE Cinema!", {
  ...
  return isOpenLogin ? (
    <div className="fixed bottom-0 left-0 right-0 top-0 z-10 bg-black/50">
      <div className="relative flex h-full w-full flex-col items-center justify-center gap-3">
        <div className="shadow-login mx-5 flex flex-col items-center justify-center rounded-lg border border-slate-400 border-transparent p-5 backdrop-blur-sm transition-all duration-500 hover:border-white focus-visible:outline-none md:mx-3 md:w-fit md:p-10">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="w-full">
            <div className="mx-auto my-3">
              ...
              <div className="relative flex items-center">
                <input
                  className="border-kube-darkblue focus:border-kube-yellow focus:ring-kube-yellow block w-full rounded-lg border-[0.5px] bg-transparent p-2 pr-10 font-medium text-slate-100 focus-visible:outline-none"
                  type="text"
                  id="email"
                  {...register("email", {
                    required: {
                ...
                <FontAwesomeIcon
                  ...
                  className="text-kube-darkblue absolute right-2 h-5 w-5"
                />
              </div>
            ...
            <div className="flex flex-col items-start py-3">
              <button
                type="submit"
                disabled={isLogined}
                className="border-kube-lightblue text-kube-darkblue hover:text-kube-darkblue mx-auto w-fit rounded border px-5 py-2 text-lg font-black shadow-xl transition-all hover:bg-white/50 hover:shadow-none disabled:cursor-not-allowed disabled:opacity-50 md:text-base"
              >
